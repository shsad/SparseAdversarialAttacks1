...
def carlini_wagner_l2(
    ...
    if y is None: # --> If we give the original labels to it, it won't change y
    ...
    # Add a binary varibale which lets us know whether a pixel is used or not, 1 - is used, 0 - is not used
    no_change_values = torch.mul(x, torch.ones_like(no_change) - no_change)
